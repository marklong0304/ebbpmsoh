<?php
  function buildGrid( $aData ){
	 ...
	 if( isset( $aData['rownumbers'])) $jqGrid->setRownumbers($aData['rownumbers']);
  ...
  function buildGridData( $aData ){
      ...
      $is_incId = (isset($aData['inc_id'])) ? $aData['inc_id']:false; //use increment id, var i.
	...
	function buildGridLocal( $aData, $fData=false) {
		$CI =& get_instance();
		$myvar = new StdClass();
		...
		$myvar->this_url='';
		if( isset( $aData['this_url'] ) ) {
			$myvar->this_url = base_url().$aData['this_url'];	
		}
		...
		$myvar->grid_name = 'grid_local_'.$sess_auth->gModul;
			...
			foreach( $aData['set_columns'] as $sProperty ){
				$aProperty[] = array(
						...
						array(
								...
								'index' => $sProperty['name'],
								'align' => isset($sProperty['align']) ? $sProperty['align'] : '',
								'width' => isset($sProperty['width']) ? $sProperty['width'] : '100',
								'editable'    => true,
								'hidden' => isset($sProperty['hidden']) ? $sProperty['hidden'] : FALSE,
						)
				);
		...
		$myvar->form_name = '';
		$myvar->form_content = '';
		if($fData) { //include form if exists
			$myvar->form_name = 'form_local_'.$sess_auth->gModul;
			
			$myvar->form_content = '';
			if(isset($fData['form_content'])) {
				foreach($fData['form_content'] as $key => $form) {
					$myvar->form_content.= '<div class="form_local">';
					$myvar->form_content.= $form;
					$myvar->form_content.= '</div>';
				}
				
			}
		}
		
		$myvar->submit_url = isset($aData['submit_url'])?base_url().$aData['submit_url']:base_url().'myform_save/save_hd';
		
		$myvar->hidden_fld = '';
		$myvar->hid_fld_sel = 'hidden_fld_'.$sess_auth->gModul;
		if( isset($aData['hidden_fld']) ) {
			foreach( $aData['hidden_fld'] as $fld_name => $fld_val ) {
				if($fld_name=='grid_local_id' && $fld_val) {
					$myvar->url = $myvar->this_url.'/buildGridLocalData/'.$fld_val;
				}
				$myvar->hidden_fld.='<input type="hidden" class="'.$myvar->hid_fld_sel.'" name="'.$fld_name.'" value="'.$fld_val.'"/>';
			}
		}
		
		$data['no_grid'] = $sess_auth->gModul;
		$data['this_url'] = $myvar->this_url;
		$data['url'] = $myvar->url;
		$data['grid_name'] = $myvar->grid_name;
		$data['pager_name'] = $myvar->pager_name;
		$data['columns'] = $myvar->columns;
		$data['form_name'] = $myvar->form_name;
		$data['form_content'] = $myvar->form_content;
		$data['submit_url'] = $myvar->submit_url;
		$data['caption'] = $myvar->caption;
		$data['hidden_fld'] = $myvar->hidden_fld;
		$data['hid_fld_sel'] = $myvar->hid_fld_sel;
