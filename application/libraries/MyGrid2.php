...
class MyGrid2 { 
	...
	public function drawGrid($property) { 
		...
		foreach($property['data'] as $para) {						
			...
			if (isset($para['adv_src']) && $para['adv_src'] === TRUE) {
				$iAdvSrc++;
			}
		...
		$view		= $property['view'];
		$no_urut    = (isset($property['no_urut']) ? $property['no_urut'] : '');
		...
		foreach($paramx as $par) {	
			...
			//echo $par['width'];
			$align = (isset($par['align']) ? $par['align'] : 'left');
				
			$aoColumns .= "{'sClass':'{$align}', 'sTitle':'".$par['label']."', 'sWidth':'".$par['width']."px', 'bVisible' : $visible, 'mData':$i},";
			$i++;
		}
		
		$aoColumns .= "{'sClass': 'center','sTitle': 'Action', 'bSortable': false, 'bSearchable': false, 'sWidth': '2%', 'mData': $i}";
		...
		if ($acl != 0) {			
			...
			$advSrc_Type = '';
			if ($iAdvSrc > 0) {
				$data['appGrid'] .= "<div id='adv_src_".$property['pk_tbl']."'>";
				$data['appGrid'] .= "<table width='100%' border='0' cellpadding='3' cellspacing='0'>";
				$data['appGrid'] .= "<tr><td height='25' class='headerTable fg-toolbar ui-toolbar ui-widget-header ui-corner-tl ui-corner-tr ui-helper-clearfix' colspan='2'>";
				$data['appGrid'] .= "<span class='dataTables_title'>&nbsp;Search ".$property['caption']."</span></td></tr>";
				$data['appGrid'] .= "<tr><td class='ui-state ui-state-default ui-corner-bottom'>";
				
				$data['appGrid'] .= "<table width='100%' border='0' cellpadding='3' cellspacing='0' class='display'>";
				foreach($property['data'] as $para) {
					if (isset($para['adv_src']) && $para['adv_src'] === TRUE) {					
						$param = array(
										'type'=>$para['type'],
										'label'=>$para['label'],
										'name'=>'q_'.(isset($para['table']) ? $para['table'] : $property['pk_tbl'])."_".$para['name'],
										'values'=> isset($para['values']) ? $para['values'] : "",
										'relationAt' => isset($para['relationAt']) ? $para['relationAt'] : "",
										'method' => isset($para['method']) ? $para['method'] :"",
										'model'  => isset($para['model']) ? $para['model'] : "",
										'info'   => isset($para['info']) ? $para['info'] : "",
										'value'  => isset($para['value']) ? $para['value'] : "",
										'size'   => isset($para['size']) ? $para['size'] : 0,
										'required' => 0,
										'class'  => 'fieldTextBox',
										'rows' => isset($para['rows']) ? $para['rows'] : 0,
										'cols' => isset($para['cols']) ? $para['cols'] : 0,
								);
						$data['appGrid'] .= $this->_ci->myform->drawSearching($param);						
						$advSrc .= "'q_".(isset($para['table']) ? $para['table'] : $property['pk_tbl'])."_".$para['name']."',";
						$advSrc_Type .= "'".$para['type']."',";
					}
				}
				$advSrc = substr($advSrc, 0, strlen($advSrc) - 1);
				$advSrc_Type = substr($advSrc_Type, 0, strlen($advSrc_Type) - 1);
				
				$data['appGrid'] .= "<tr><td></td><td>";
				$data['appGrid'] .= "<input type='button' name='search_crud_".$no_grid.$no_urut."' id='search_crud_".$no_grid.$no_urut."' value='Search'/>";
				$data['appGrid'] .= "</td></tr>";
				$data['appGrid'] .= "</table>";
				$data['appGrid'] .= "</tr></td>";				
				$data['appGrid'] .= "</table>";
			...
			//searching
			$data['appGrid'] .= "<div style='height:5px;'></div>";
			...
			$data['appGrid'] .= "$(document).ready(function() { 
									...
									company_id	= '".$id_PT."';
									advSrc      = [".$advSrc."];
									advSrc_Type = [".$advSrc_Type."];
									
									for(i=0;i<advSrc.length;i++) {
										//sWhere1 += '\''+$('#'+advSrc[i]).val()+'\',';
										sWhere1 += $('#'+advSrc[i]).val()+',';
									}									
									sWhere1 = sWhere1.substring(0, sWhere1.length - 1);									
								";
			...
			$data['appGrid'] .= "	var aoColumns = [".$aoColumns."];
									...
									oTable = $('#grid_".$no_grid.$no_urut."').dataTable({
										...
										'fnServerParams' : function(aoData) {
											aoData.push(
												...
												{'name':'sWhere', 'value':advSrc},
												{'name':'sWhere1', 'value':sWhere1},
												{'name':'advSrc_Type', 'value':advSrc_Type});
											},
										'aoColumns'		 : aoColumns,
									...
									$('#grid_".$no_grid.$no_urut."_wrapper .headerTable').append(\"<span class='dataTables_title'>List ".$caption."</span>\");
								";						
			...
			//Add Search JS Button
			$data['appGrid'] .= "			
						$('#search_crud_".$no_grid.$no_urut."').click(function() {
							sWhere1 = '';
							for(i=0;i<advSrc.length;i++) {
								//sWhere1 += '\''+$('#'+advSrc[i]).val()+'\',';
								sWhere1 += $('#'+advSrc[i]).val()+',';
