...
class jqgrid_lib {
	...
	public function setRownumbers($rownumbers = FALSE){
		$this->rownumbers = (bool)$rownumbers;
	}
	...
	public function buildGrid() {
		...
		$buildRownumbers = ($this->rownumbers)?'true':'false';
		...
		//NavBar
		$grid .= "$('#$buildDivName').jqGrid('navGrid', '#pager_".$sess_auth->gModul."',
      ...
      if( !empty( $buildCustomButtons ) ){
         foreach($buildCustomButtons as $customButton) {
               $customButton = ".navButtonAdd('#grid_".$sess_auth->gModul."_toppager_left',".$customButton.")";
               $grid .= $customButton;
            }
      }

		$grid .= ".navButtonAdd('#grid_".$sess_auth->gModul."_toppager_left',
					{ caption:'', buttonicon:'ui-icon-trash', onClickButton:jqGridDelete ,title: 'Delete selected row', position: 'first', cursor: 'pointer'})
					.navButtonAdd('#grid_".$sess_auth->gModul."_toppager_left',
