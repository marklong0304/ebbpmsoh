 ...
 */
(function(a) {
	a.ttwNotificationMenu = function(c) {
		var l, b, x, y = this, w = {}, A = {}, n = {}, g, s, q, p, m, e, o, f = 0;
		l = {
			notificationList : {
				showMenu : true,
				anchor : "bubble",
			...
			deleteCallback : function(B) {
			},
			notificationCountIncrease : function(B) {
			},
			notificationClickCallback : function(B) {
			}
		};
		x = {
			tmp : '<div id="tmp"></div>',
		...
		function t(B, C) {
			return ( typeof A[B] != "undefined") && (B != b.defaultCategory)
		}

		function d(C) {
			var B = C.read ? "read" : "unread";
			delete w[C.category][B][C.id];
			delete C
		}
		...
		function j(B) {
			return (a.inArray(B, ["read", "unread", "all"]) != -1)
		}

		function k(B) {
			return (( typeof B != "undefined") ? B : "") + new Date().getTime() + "-" + Math.floor(Math.random() * (100000 - 1 + 1)) + 1
		}
			...
			if(E != "all" && D != "all") {
				...
				if(E == "all" && D == "all") {
					...
					for(var B = 0; B < p.length; B++) {
						a.extend(C, w[E[B]].read, w[E[B]].unread);
						a.extend(F, C)
					}
					return F
				} else {
					if(E == "all" && D != "all") {
						...
						if(E != "all" && D == "all") {
							return a.extend({}, w[E].read, w[E].unread)
						} else {
							return false
						}
					}
		...
		g.prototype.init = function() {
			...
			if(b.showNotificationList) {
				this.$bubble.bind("click", function() {
					if(!B.notificationList) {
						B.notificationList = new q({
							category : B.category,
							$anchor : (b.notificationList.anchor == "bubble") ? B.$bubble : B.$item
						})
					}
				})
			}
		...
		g.prototype.createBubble = function() {
			this.$bubble = a(x.notificationBubble).html("0").appendTo(this.$item);
		...
		g.prototype.updateBubble = function() {
			var B = (w[this.category]) ? w[this.category].unreadCount : 0;
			this.$bubble.html(B);
			if(B <= 0 && !b.bubble.showEmptyBubble) {
				this.$bubble.stop().fadeOut("fast")
			}
			if(B > 0 && (n[this.category] == 0)) {
				this.$bubble.stop().fadeIn()
			}
			if(n[this.category] < B) {
				z(b.notificationCountIncrease, this)
			}
			n[this.category] = B
		};
		q = function(C) {
			...
			this.type = "unread";
			this.markup = {
				notificationListWrapper : '<div class="notification-list-wrapper" style="z-index:9999999;"></div>',
			...
			this.settings = a.extend(true, {}, b.notificationList, C);
			if(o) {
				var B = this;
				o.close(function() {
					B.build()
		...
		q.prototype.build = function() {
			...
			if(this.settings.showMenu) {
				B = a(this.markup.notificationListMenu);
		...
		q.prototype.populate = function() {
			var D, C = this, E, B;
			D = a(this.markup.notificationList).attr("data-type", C.type);
			E = r(this.settings.category, this.type);
			if(!a.isEmptyObject(E)) {
				a.each(E, function(F, G) {
					B = G.settings.icon ? "show-icon" : "";
					a(C.markup.notificationListItem).addClass(B).append(G.getHtml()).data({
						id : G.id,
						category : G.category,
						notification : G
					}).appendTo(D)
				...
				a(C.markup.notificationListItem).addClass("empty-list").html(b.notificationListEmptyText).appendTo(D)
			}
			D.find(e.notificationListItem).bind("click", function() {
				z(b.notificationClickCallback, a(this).data("notification"))
			});
			if(this.$list) {
				this.$list.remove()
			}
		...
		q.prototype.bindMenuActions = function() {
			var C = this, B;
			this.$wrapper.find(e.notificationListMenuItem).bind("click", function() {
				C.type = a(this).attr("id").split("-")[0];
				C.populate()
			});
			this.$wrapper.delegate(".close", "click", function() {
				var D = a(this);
				D.parents(e.notificationListItem).animate({
					opacity : 0
				}, 400, function() {
					var F = a(this), E = F.data();
					if(w && w[E.category] && w[E.category][C.type]) {
						if(w[E.category][C.type][E.id]) {
							B = w[E.category][C.type][E.id].markRead();
							A[E.category].updateBubble()
						}
					}
					F.remove();
					z(b.markReadCallback, B)
				})
			});
		...
		q.prototype.close = function(C) {
			...
			this.$wrapper.fadeOut(50, function() {
				B.$wrapper.remove();
				A[B.settings.category].notificationList = false;
				o = false;
				z(C)
			})
		};
		...
		s.prototype.create = function() {
			if(this.settings.message) {
				this.id = this.settings.id || k("notification");
		...
		s.prototype.html = function() {
			this.$notification = a(x.notification);
		...
		s.prototype.setValues = function() {
			if( typeof this.settings.icon != "undefined") {
				this.$notification.addClass("show-icon").find(".icon").css("background", "transparent url(" + this.settings.icon + ") no-repeat center center scroll").html(x.notificationIcon).find("img").attr("src", this.settings.icon)
			}
			if( typeof this.message != "undefined") {
				this.$notification.find(".message").html(this.message)
			}
		};
		s.prototype.markRead = function() {
			this.read = true;
			w[this.category].readCount++;
			w[this.category].read[this.id] = this;
			w[this.category].unreadCount--;
			delete w[this.category].unread[this.id];
			return this
		};
		s.prototype.destroy = function() {
			d(this)
		};
		y.createNotification = function(C) {
			var B = new s(C);
			if(B.create()) {
				u(B);
				z(b.createCallback, B)
			}
			return B
		};
		y.initMenu = function(B) {
			a.each(B, function(E, C) {
				var D = new g(E, C);
				h(E, D)
			})
		};
		y.getNotifications = function(C, B) {
			return r(C, B)
		};
		y.deleteNotification = function(B) {
			B.destroy()
		};
		y.notifications = w;
		v();
