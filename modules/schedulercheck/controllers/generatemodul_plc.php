...
class generatemodul_plc extends MX_Controller {
    ...
    function index($action = '') {
        ...
        $grid = new Grid;
        $grid->setTitle('CRUD Master FOR PLC'); 
        $grid->setTable('hrd.scheduler_group_pic');     
        $grid->setUrl('generatemodul_plc'); 
        
        switch ($action) {
                case 'piew': 
                        $data = array(
                            'url'=>base_url().'processor/schedulercheck/generatemodul_plc?action=write',
                            'url2'=>base_url().'processor/schedulercheck/generatemodul_plc?action=addTable',
                            'url3'=>base_url().'processor/schedulercheck/generatemodul_plc?action=getField',
...
//$modu['vKode_modul'] = kodeInHere
$modu['vNama_modul'] = $createname_space;
$modu['vDesciption'] = $createname_space;
$modu['dCreate'] = date('Y-m-d'); 
$this->db_schedulercheck->insert('plc3.m_modul',$modu);
 
$idModul = $this->db_schedulercheck->insert_id();
...
$html .="
    /*
        ...
        function confirm_view() {
            ...
            \$echo .= '<form id=".'"'."form_".$createname_space."_confirm".'"'." action=".'"'."'.base_url().'processor/".$openModule."/".$createname_space."?action=confirm_process".'"'." method=".'"'."post".'"'.">';
            ...
            <button type=".'"'."button".'"'." onclick=".'"'."submit_ajax(\'form_".$createname_space."_confirm\')".'"'.">Confirm</button>';
...
$html .="
    ...
    function before_insert_processor(\$row, \$postData) {
        ...
        \$controller_name ='".$createname_space."';
        \$pk_field = '".$field_pk."';
...
$html .="
    function manipulate_update_button(\$buttons, \$rowData) {   
        ...
        if (\$this->input->get('action') == 'view') {
            ...
            \$isOpenEditing = \$this->lib_plc->getOpenEditing(\$this->modul_id,\$peka);
            if($isOpenEditing){
               \$update_draft = '<button onclick=".'"'."javascript:update_draft_btn(\'".$createname_space."\', \' '.base_url().'processor/folder_app/".$createname_space."?draft=true \',this,true )".'"'."  id=".'"'."button_update_draft_".$createname_space.'"'."  class=".'"'."ui-button-text icon-save".'"'." >Update open Editing</button>';
                ...
                \$approve = '<button onclick=".'"'."javascript:load_popup(\' '.base_url().'processor/folder_app/".$createname_space."?action=approve&iM_activity='.\$act['iM_activity'].'&".$field_pk."='.\$peka.'&company_id='.\$this->input->get('company_id').'&group_id='.\$this->input->get('group_id').'&modul_id='.\$this->input->get('modul_id').' \')".'"'."  id=".'"'."button_approve_".$createname_space."".'"'."  class=".'"'."ui-button-text icon-save".'"'.">Approve</button>';
                \$reject = '<button onclick=".'"'."javascript:load_popup(\' '.base_url().'processor/folder_app/".$createname_space."?action=reject&iM_activity='.\$act['iM_activity'].'&".$field_pk."='.\$peka.'&company_id='.\$this->input->get('company_id').'&group_id='.\$this->input->get('group_id').'&modul_id='.\$this->input->get('modul_id').' \' )".'"'."  id=".'"'."button_reject_".$createname_space."".'"'."  class=".'"'."ui-button-text icon-save".'"'." >Reject</button>'; 

                \$confirm = '<button onclick=".'"'."javascript:load_popup(\' '.base_url().'processor/folder_app/".$createname_space."?action=confirm&iM_activity='.\$act['iM_activity'].'&".$field_pk."='.\$peka.'&company_id='.\$this->input->get('company_id').'&group_id='.\$this->input->get('group_id').'&modul_id='.\$this->input->get('modul_id').' \' )".'"'."  id=".'"'."button_confirm".$createname_space."".'"'."  class=".'"'."ui-button-text icon-save".'"'." >Confirm</button>'; 
