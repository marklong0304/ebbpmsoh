...
<?php
/* Generated by Softdev 2 ERP Module Generator 2019-02-01 21:40:25 */
...
class mt01 extends MX_Controller {
    ...
    function index($action = '') {
        ...
        //List Table
        $grid->addList('vNo_transaksi','vNomor','vPerihal','dTanggal','iCustomer','vNama_produsen','iM_tujuan_pengujian','vNama_sample','vRemark','iSubmit','iApprove'); 
        ...
        switch ($action) {
            ...
            case 'createproses':
                ...
                if($isUpload) {
                    ...
                    $key = $this->input->post('vNomor');
                    $cek_data = 'select * from bbpmsoh.mt01 a where a.lDeleted=0 AND a.vNomor="'.$key.'" ';
                    $data_cek = $this->db->query($cek_data)->row_array();
                    if (empty($data_cek) ) {
                        echo $grid->saved_form();     
                    }else{
                        $r['status'] = FALSE;
                        $r['message'] = "Nomor Surat Sudah ada";
                        echo json_encode($r);
                    }
            ...
            case 'updateproses':
                    ...
                    if($isUpload) {
                        ...
                        $cek_data = 'select * from bbpmsoh.mt01 a where a.vNomor="'.$key.'" and a.lDeleted=0 ';
                        $result = $this->db->query($cek_data)->row_array();

                        $sql2 = 'select * from bbpmsoh.mt01 a where a.iMt01 = "'.$id.'"';
                        $old = $this->db->query($sql2)->row_array();


                        if (empty($result) or $old['vNomor'] == $key  ) {
                                echo $grid->updated_form();
                        }else{
                            $r['status'] = FALSE;
                            $r['message'] = "No Surat Sudah ada";
                            echo json_encode($r);
