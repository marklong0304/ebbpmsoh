...
class mt07 extends MX_Controller {
    ...
    function get_data_prev(){
    	...
    	$this->db->select("mt01.*,m_tujuan_pengujian.cKode,mt07.*")
    				->from("bbpmsoh.mt07")
		...
		if($q->num_rows()){
			foreach ($q->result() as $k) {
				...
				foreach ($rsel as $dsel => $vsel) {
					if($dsel=="iAction"){
						...
						$dataar[$z]="<input type='text' name='".$this->url."_".$dsel."' id='grid_details_".$nmTable."_".$dsel."_".$i."' value='".$k->{$dsel}."' class='required' size='25'>";
					}
					$z++;
				}
			...
			foreach ($rsel as $dsel => $vsel) {
				if($dsel=="iAction"){
					...
					$dataar[$z]="<input type='hidden' class='num_rows_".$nmTable."' value='".$i."' /><input type='text' name='grid_details_nomor_request[0][]' id='grid_details_nomor_request_".$i."' value='' class='get_sample_req_".$nmTable." required' size='25'><input type='hidden' name='".$this->url."_iMt01' id='grid_details_".$nmTable."_iMt01_".$i."' value='' class='required' size='25'>";
				}else{
					$dataar[$z]="<input type='text' name='".$this->url."_".$dsel."' id='grid_details_".$nmTable."_".$dsel."_".$i."' value='' class='required' size='25'>";
				}
				$z++;
			}
			$data->rows[$i]['cell']=$dataar;
