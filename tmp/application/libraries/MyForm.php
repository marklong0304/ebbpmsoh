...
class MyForm {
	...
	public function drawForm($param, $property, $group_and_modul="") {
		...
		$drawForm .= '<input type="hidden" class="para" name="'.$table.'_lastUpdateAt" id="'.$table.'_lastUpdateAt" value="'.$lastUpdate['tUpdated'].'"/>';
		...
		$drawForm .= '<tr>';
		$drawForm .= '<td colspan="2" align="center">';	
		...
		if ($proc == 'edit' || $proc == 'add')	{
			...
			if (!isset($button) || $button == '') {
				...
				$drawForm .= $this->setSave("submit", "Save", $url, $required_field, $duplicated_field, $prop_tbl, $form_element_to_save, $form_type_to_save);
	...
	public function setSave($name, $label, $url, $required_field, $duplicated_field, $prop_tbl, $form_element_to_save, $form_type_to_save) { // bikin button submit
		$opt = array(
		...
		<script language='text/javascript'>
			$(document).ready(function() {
				...
				$('#cancel_".$this->no_grid.$this->options['no_urut']."').click(function() {	
					...
					$.ajax({
						'type' : 'GET',
						'data': 'company_id=".$this->company."&modul_id=".$group_and_modul['modul_id']."&group_id=".$group_and_modul['group_id']."&parent_id=".$group_and_modul['parent_id']."&urut=".$this->options['no_urut']."',						
						'url' : '".$base.$url."/output',
						'success' : function(data) {
									//alert(no_grid);
									$('#tab_'+no_grid).html(data);
